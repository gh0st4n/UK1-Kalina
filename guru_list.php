...
<div class="container-fluid px-4 mt-4 mb-5">
    ...
    <?php if ($msg === 'added'): ?>
        <div class="alert alert-success alert-dismissible fade show shadow-sm" role="alert">
            <i class="fa-solid fa-circle-check me-2"></i>Data guru berhasil ditambahkan!
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>
    <?php elseif ($msg === 'updated'): ?>
        <div class="alert alert-info alert-dismissible fade show shadow-sm" role="alert">
            <i class="fa-solid fa-circle-check me-2"></i>Data guru berhasil diperbarui!
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>
    <?php elseif ($msg === 'deleted'): ?>
        <div class="alert alert-warning alert-dismissible fade show shadow-sm" role="alert">
            <i class="fa-solid fa-circle-check me-2"></i>Data guru berhasil dihapus!
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>
    <?php endif; ?>
    ...
    <div class="d-flex justify-content-between align-items-center p-3 mb-4 bg-white rounded-3 shadow-sm border-start border-4 border-primary">
        <div>
            <h4 class="fw-bold mb-1 text-dark"><i class="fa-solid fa-user-tie text-primary me-2"></i>Data Guru</h4>
            <p class="text-muted mb-0 small">Kelola daftar pengajar dan mata pelajaran yang diampu.</p>
        </div>
        <?php if ($user_role === 'admin'): ?>
            <a href="guru_tambah.php" class="btn btn-primary px-3 shadow-sm"><i class="fa-solid fa-plus me-1"></i> Tambah Guru Baru</a>
        <?php endif; ?>
    </div>
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge bg-primary-subtle text-primary border border-primary-subtle px-3 py-2 fs-6 fw-normal">
                                            <i class="fa-solid fa-user-large me-1"></i> <?= htmlspecialchars($row['nama_guru']); ?>
                                        </span>
                                    </td>
                                    <td class="text-secondary"><?= htmlspecialchars($row['mapel'] ?? '-'); ?></td>
                                    ...
                                    <?php if ($user_role === 'admin'): ?>
                                        <td class="text-center">
                                            <div class="d-inline-flex gap-1 align-items-center justify-content-center">
                                                <a href="guru_edit.php?id=<?= $row['id']; ?>" class="btn btn-warning btn-sm text-white shadow-sm">
                                                    <i class="fa-solid fa-pen-to-square"></i> Edit
                                                </a>
                                                
                                                <form action="guru_hapus.php" method="POST" class="d-inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data guru ini?');">
                        ...
                        <?php else: ?>
                            <tr>
                                <!-- Colspan 5 untuk Admin, Colspan 3 untuk User (No, Nama Guru, Mata Pelajaran) -->
